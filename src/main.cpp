...
{  
  switch (dir)
  {
    case'L':
      ...
      {
        qtra.readLineWhite(sensors1);    //Move left until left extreme sensor goes out of the white line comes on black
      }
      qtra.readLineWhite(sensors1);
      left(motor1, motor2, 200);
      while (sensors1[0] < thr[0])
      {
        qtra.readLineWhite(sensors1);        // Again move left until it get's on white line
      }
    ...
    case'R':
      ...
      {
        qtra.readLineWhite(sensors1);   //Move right until left extreme sensor goes out of the white line comes on black
      }
      qtra.readLineWhite(sensors1);
      right(motor1, motor2, 200); //150
      while (sensors1[7] < thr[7])
      {
        qtra.readLineWhite(sensors1);         // Again move right until it get's on white line
      }
